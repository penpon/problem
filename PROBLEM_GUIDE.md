...
### 手順4: タブ表示用アイコンの追加（オプション）

新しいカテゴリにカスタムアイコンを設定したい場合は、`js/main.js`を編集します：

```javascript
// js/main.js の createTabs メソッド内
const categoryIcons = {
  '総合': '🌟',
  'オブジェクト': '📦',
  '関数': '⚡',
  '配列': '📋',
  '条件分岐': '🔀',    // ← 新しいカテゴリのアイコンを追加
  'ループ': '🔄',
  '基本': '🎯'
};
```
...
**アイコン設定手順：**
1. `js/main.js`ファイルを開く
2. `createTabs`メソッド内の`categoryIcons`オブジェクトを見つける
3. 新しいカテゴリ名とアイコンを追加
4. ファイルを保存
...
**注意：** アイコンを設定しない場合は、デフォルトの📝アイコンが使用されます。
...
### タブ表示の仕組み
- **自動検出**: `problems/index.json`の`category`フィールドを読み取り、自動的にタブを生成
- **表示順序**: `index.json`の記載順序でタブが表示される
- **問題数表示**: 各タブに問題数が自動表示される（例：🌟 総合 (5問)）
- **アイコン表示**: `js/main.js`の`categoryIcons`で設定されたアイコンを表示

### 新カテゴリのタブ表示確認
1. `grader.html`をブラウザで開く
2. 新しいカテゴリのタブが表示されることを確認
3. タブをクリックして問題リストが表示されることを確認
4. 各問題をクリックして正常に読み込まれることを確認

### キャッシュ対策
- ブラウザキャッシュの影響で反映が遅れる場合は、以下を試す：
  - **強制リロード**: `Ctrl+F5` (Windows) / `Cmd+Shift+R` (Mac)
  - **キャッシュクリア**: ブラウザの開発者ツール → Network → Disable cache
  - **プライベートモード**: シークレット/プライベートブラウジングで確認
