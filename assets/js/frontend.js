...
extractProblemNumber(problemId, categoryId) {
    ...
    displayProblemDetails(problem) {
        ...
        // description に instructions を追記（テキストはすべてエスケープ）
        const descriptionParts = [];
        if (problem.description) descriptionParts.push(escapeHtml(problem.description));
        const inst = (problem.instructions || [])
            .map(i => i ? `・${escapeHtml(i)}` : '')
            .filter(Boolean)
            .join('<br>');
